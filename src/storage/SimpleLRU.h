...
private:
    ...
    typedef std::map<std::reference_wrapper<const std::string>, std::reference_wrapper<lru_node>, std::less<std::string>> map_type;

    void release_memory(size_t &size);
    void set_process(const map_type :: iterator &pair, const std :: string &value);
    void put_process(const std :: string &key, const std :: string &value);
